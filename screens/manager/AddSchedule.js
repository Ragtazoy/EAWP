...
const AddSchedule = ({ route, navigation }) => {
   ...
   const AddWorkSchedule = async () => {
      ...
      selectedCashier.map((val) => {
         axios.post('http://10.0.2.2:81/create/scheduling', { emp_id: val, dept: 'cashier' }).then(() => {
            console.log('post /create/scheduling already')
         })
      })
   ...
   return (
      <NativeBaseProvider>
         <Header icon={'faCalendarDay'} color={'amber.500'} title={'เลือกพนักงาน'} subtitle={moment(date.dateString).format('DD MMMM YYYY')} element={propHeader()} />
